...
def evaluate_single_omr(upload_path, filename, student, subject_id):
    ...
    # Number of questions detected vs number of keys provided
    total_detected = total_q
    total_key_questions = len(answer_key)
    
    # If the student only scanned 3 questions, the score should likely be out of 3 
    # unless there are more answers in the key that they simply missed.
    # We use the count of detected questions as the baseline if it's smaller.
    effective_total = total_detected if total_detected < total_key_questions and total_detected > 0 else total_key_questions
    if effective_total == 0: effective_total = total_detected
    
    percentage = (score / effective_total * 100) if effective_total > 0 else 0
    ...
    new_result = Result(
        ...
        status=status,
        answers_json=json.dumps(selected)
    )
